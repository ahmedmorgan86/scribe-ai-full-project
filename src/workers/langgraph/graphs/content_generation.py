...
def get_llm(model_type: Literal["fast", "quality"] = "quality") -> ChatAnthropic | ChatOpenAI:
    """Get LLM instance based on task type."""
    config = get_config()

    if model_type == "fast" and config["openai_api_key"]:
        return ChatOpenAI(
            model="gpt-4o-mini",
...
def voice_check(state: GraphState) -> GraphState:
    """
    Validate content against voice corpus in Qdrant.
    ...
    try:
        ...
        try:
            ...
            if "approved_posts" in collection_names:
                try:
                    count = qdrant.count("approved_posts")
                    if count.count > 0:
                        pass
                except Exception:
                    pass
            ...
            if not similar_posts:
                ...
                state["voice_similarity"] = 0.0
                state["voice_feedback"] = "No voice corpus available - skipping check"
                state["similar_posts"] = []
                add_trace(state, "voice_check", "skip", {"reason": "no_corpus"}, start_time)
                return state

            state["similar_posts"] = similar_posts
            state["voice_similarity"] = max_similarity

            if max_similarity >= VOICE_SIMILARITY_THRESHOLD:
                state["voice_check_passed"] = True
                state["voice_feedback"] = f"Voice match: {max_similarity:.1%}"
            else:
                state["voice_check_passed"] = False
                state["voice_feedback"] = f"Voice similarity {max_similarity:.1%} below {VOICE_SIMILARITY_THRESHOLD:.0%} threshold"

            add_trace(state, "voice_check", "success" if state["voice_check_passed"] else "fail",
                     {"similarity": max_similarity, "threshold": VOICE_SIMILARITY_THRESHOLD}, start_time)

        except Exception as e:
            logger.warning(f"Qdrant query failed: {e}")
            state["voice_check_passed"] = True
            state["voice_similarity"] = 0.0
            state["voice_feedback"] = "Voice check skipped - Qdrant unavailable"
            state["similar_posts"] = []
            add_trace(state, "voice_check", "skip", {"reason": str(e)}, start_time)

    except Exception as e:
        logger.error(f"voice_check error: {e}")
        state["voice_check_passed"] = True
        state["voice_similarity"] = 0.0
...
def slop_check(state: GraphState) -> GraphState:
    ...
    banned_phrases = [
        "let's dive in", "here's the thing", "game-changer", "game changer",
        "in today's world", "at the end of the day", "it's worth noting",
        "buckle up", "without further ado", "let's get started",
        "in this thread", "thread:", "1/", "🧵", "a thread",
        "first of all", "secondly", "lastly", "in conclusion",
        "to sum up", "the key takeaway", "hot take:",
    ]
    ...
    if numbered_lines >= 3:
        issues.append("Listicle format detected")
        score += 15

    if "reddit" in content_lower or "r/" in content_lower:
        issues.append("References Reddit")
        score += 25
    if "hackernews" in content_lower or "hacker news" in content_lower or "hn" in content_lower.split():
        issues.append("References HackerNews")
        score += 25

    score = min(100, score)
    passed = score < SLOP_SCORE_THRESHOLD

    state["slop_check_passed"] = passed
    state["slop_score"] = score
    state["slop_issues"] = issues
    state["slop_feedback"] = "Clean content" if passed else f"Slop score {score}/100: {', '.join(issues)}"

    add_trace(state, "slop_check", "success" if passed else "fail",
             {"score": score, "issues": issues}, start_time)

    return state


STYLOMETRIC_THRESHOLD = 0.7
...
def stylometric_check(state: GraphState) -> GraphState:
    ...
    try:
        ...
        response = httpx.post(
            ...
            json={
                "content": content,
                "threshold": STYLOMETRIC_THRESHOLD,
            },
            timeout=30.0,
        )
